...
export const portalRouter = router({
  ...
  /**
   * Buscar dados de Legislativo (TCE-RS) - Gestão Fiscal das Câmaras Municipais 2021-2025
   * Fonte: Tribunal de Contas do Estado do Rio Grande do Sul
   * Limite constitucional: 6% da RCL (art. 29-A da CF)
   */
  legislativoTCERS: publicProcedure
    .input(
      z.object({
        codigoIBGE: z.string().optional(),
      })
    ...
    .query(async ({ input }) => {
      try {
        ...
        if (!fs.existsSync(legislativoPath)) {
          return null;
        }
        const legislativoData = JSON.parse(fs.readFileSync(legislativoPath, 'utf-8'));
        ...
        return legislativoData[municipioNome] || null;
      } catch (error) {
        console.error('Erro ao carregar dados de Legislativo (TCE-RS):', error);
        return null;
      }
    }),

  /**
   * Buscar dados de Saúde (TCE-RS) - Índice de Aplicação em ASPS 2021-2025
   * Fonte: Tribunal de Contas do Estado do Rio Grande do Sul
   * Mínimo constitucional: 15% (EC 29/2000)
   */
  saudeTCERS: publicProcedure
    .input(
      z.object({
        codigoIBGE: z.string().optional(),
      })
    ...
    .query(async ({ input }) => {
      try {
        ...
        const saudeePath = path.join(process.cwd(), 'public', 'saude-tce-rs.json');
        if (!fs.existsSync(saudeePath)) {
          return null;
        }
        const saudeData = JSON.parse(fs.readFileSync(saudeePath, 'utf-8'));
        ...
        return saudeData[municipioNome] || null;
      } catch (error) {
        console.error('Erro ao carregar dados de Saúde (TCE-RS):', error);
        return null;
      }
    }),
